...
import { bookBusSeats } from '../../services/bookingServices';
import back_icon from '../../assets/back_icon.svg';
import { toast, ToastContainer } from 'react-toastify';
function BusPassengerDetailsPage() {
  const location = useLocation();
  const navigate = useNavigate();
  const { userId, bus, seats, busId, journyStartDate } = location.state || {};

  if (!userId || !bus || !seats || !busId) {
  ...
  return (
    <div className="container mt-4">
      ...
      <div style={{ display: "flex", justifyContent: "flex-start", gap: 20 }}>
        ...
        <h2>Passenger Details for {bus.busName}</h2>
      </div>
...
export default BusPassengerDetailsPage;
